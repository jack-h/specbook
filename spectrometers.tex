...
Time resolution refers to the minimum integration time, $t_{\rm{min}}$, over which a spectrometer computes the time average. Detection of transient phenomena, such as fast radio bursts %\cite{Lorimer2007}
 and pulsars, require $t_{\rm{min}}$ to be as short as a microsecond, whereas integration lengths of several seconds are common when observing faint sources.  %The minimum and maximum averaging times are generally enforced by hardware limitations.
...
If downsampling a FIR filtered signal by $\downarrow D$, we only keep the outputs $n=qD$. %A downsampled filter aliases will alias spectra centered at any multiple of the output sample rate to baseband. 
